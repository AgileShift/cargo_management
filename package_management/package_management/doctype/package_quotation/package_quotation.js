...
function calculate_quotation_total_import(frm) {
    // Calculate the 'total_import_cost' field on Package Quotation Doctype(Parent)
...
frappe.ui.form.on('Package Quotation', {
    onload: function (frm) {
    ...
    refresh: function(frm) {
        ...
        frm.add_custom_button(__('Render Quotation'), () => {
            ...
            $(frm.fields_dict['text'].wrapper)
                .html(frappe.render_template('package_quotation_details', {doc: doc_to_render}));
...
frappe.ui.form.on('Package Quotation Item', {
    // Children doctype of Package Quotation
